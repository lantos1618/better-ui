...
const complexTool = aui
  ...
  .clientExecute(async ({ input, ctx }) => {
    ...
    const result = await ctx.fetch('/api/aui/aui-execute', { 
      method: 'POST',
...
// AI-optimized tool with middleware
const aiTool = aui
  .tool('ai-assistant')
  ...
  .middleware(async ({ input, ctx }) => {
    ...
    console.log(`AI Request: ${input.prompt}`);
  })
...
const databaseTool = aui
  ...
  .middleware(async ({ input, ctx }) => {
    ...
    if (input.operation !== 'find' && !ctx.user) {
...
// Demo component
export default function AUIConciseDemo() {
  ...
  const runExamples = async () => {
    ...
    try {
      ...
      // Show AI discovery
      console.log('Available tools for AI:', aui.list());
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold mb-6">AUI - Concise API</h1>
      ...
      <div className="mb-6 p-4 bg-blue-50 rounded">
        ...
        <ul className="list-disc ml-6 text-sm">
          ...
          <li>Middleware support for validation & logging</li>
...
{`const complexTool = aui
  ...
  .render(({ data }) => <SearchResults results={data} />)`}
        ...
        <div className="border p-4 rounded">
          <h2 className="text-xl font-semibold mb-2">AI Assistant with Middleware</h2>
          <pre className="bg-gray-100 p-2 rounded text-sm mb-4">
{`const aiTool = aui
  .tool('ai-assistant')
  .input(z.object({ prompt: z.string() }))
  .middleware(async ({ input }) => console.log(input))
  ...
  .render(({ data }) => <CodeBlock code={data.code} />)`}
        ...
        <div className="border p-4 rounded">
          <h2 className="text-xl font-semibold mb-2">Calculator with Permissions</h2>
          {calcData && (
        ...
        <div className="border p-4 rounded">
          <h2 className="text-xl font-semibold mb-2">Database Tool with Middleware</h2>

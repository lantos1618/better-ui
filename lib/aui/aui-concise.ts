...
type ToolConfig<I, O> = {
  ...
  retry?: number;
  timeout?: number;
  middleware?: Array<(params: { input: I; ctx: AUIContext }) => Promise<void> | void>;
  stream?: boolean;
  permissions?: string[];
};
...
class Tool<I = any, O = any> {
  ...
  middleware(fn: (params: { input: I; ctx: AUIContext }) => Promise<void> | void): this {
  ...
  async run(input: I, ctx?: AUIContext): Promise<O> {
    ...
    if (this.config.middleware) {
      for (const mw of this.config.middleware) {
        await mw({ input: validated, ctx: context });
      }
    }

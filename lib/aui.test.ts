...
describe('AUI - Ultra Concise API', () => {
  beforeEach(() => {
    ...
    aui.list().forEach(name => {
      const tools = aui as any;
      tools.tools.delete(name);
    });
  });

  describe('Simple Tool Pattern', () => {
    it('creates a tool with just execute and render', () => {
      const tool = aui
        .tool('simple')
  ...
  describe('Tool Registry', () => {
    it('registers tools automatically', () => {
      aui.tool('tool1');
      aui.tool('tool2');
      aui.t('tool3'); // shorthand

      expect(aui.list()).toContain('tool1');
      expect(aui.list()).toContain('tool2');
      expect(aui.list()).toContain('tool3');
      expect(aui.list().length).toBe(3);
    });
  ...
  describe('Auto-build via Proxy', () => {
    ...
    it('preserves tool identity through chaining', () => {
      const tool1 = aui.tool('chain');
      const tool2 = tool1.input(z.object({ x: z.number() }));
      const tool3 = tool2.execute(async ({ input }) => input.x);
      
      // All references point to the same tool
      expect(aui.get('chain')).toBe(tool1);
      expect(aui.get('chain')).toBe(tool2);
      expect(aui.get('chain')).toBe(tool3);
    });
  });

...
```tsx
// 😩 The old way: manually map tool outputs to UI
...
weather.view((data) => (
  <div className="p-4 rounded-xl bg-blue-50">
    ...
    <p className="text-3xl font-bold">{data.temp}°F</p>
  </div>
));
```
...
<CardGroup cols={2}>
  <Card title="Tools = Views" icon="puzzle-piece">
    Every tool defines its own React view. AI calls tool → view renders automatically.
  </Card>
...
└─────────────┘     │  ChatProvider │
                    │  Thread/Panel │
                    │  ToolResult   │──▶ tool.View auto-renders
                    └───────────────┘
```

...
interface ChatContextValue {
  ...
  // Persistence fields (only when persistence prop is configured)
  threads?: Thread[];
  threadId?: string;
...
| `retryTool` | `(id, name, input) => void` | Retry a failed tool execution |

### Persistence Fields

Only present when `ChatProvider` has a `persistence` prop:

| Field | Type | Description |
|-------|------|-------------|
| `threads` | `Thread[]` | All available threads, ordered by most recently updated |
| `threadId` | `string \| undefined` | Currently active thread ID |
| `createThread` | `(title?) => Promise<Thread>` | Create a new thread, switch to it, and clear messages |
| `switchThread` | `(id) => Promise<void>` | Switch to a different thread and load its messages |
| `deleteThread` | `(id) => Promise<void>` | Delete a thread and its messages |

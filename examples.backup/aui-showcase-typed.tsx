...
const streamingTool = aui
  ...
  .render(({ data }) => <div className="live">{(data as any).value}</div>)
  .build();
...
export function ToolDemo() {
  ...
  return (
    <div>
      ...
      {error && <span>Error: {(error as Error).message}</span>}
      {data && weatherTool.render && weatherTool.render({ data, input: { city: 'San Francisco' } })}
    </div>
  );
}
...
export async function aiHandler(query: string) {
  ...
  if (selectedTool) {
    ...
    return selectedTool.render && selectedTool.render({ data: result, input });
  }
}
